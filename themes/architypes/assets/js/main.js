document.addEventListener('DOMContentLoaded', function () {
    ...
    async function loadPage(pageNum) {
        ...
        const grid = document.querySelector('.site-main .grid');
        if (newGrid && grid) {
            // On ajoute les vignettes dans la grille existante plutôt que de créer
            // une nouvelle grille : le moteur de placement remplit ainsi les rangées
            // de façon continue, sans laisser apparaître le fond entre deux pages.
            const items = Array.from(newGrid.children);
